...
export const Target: React.FC<TargetProps> = ({ onHit }) => {
  const center = 250;
  const baseRadii = {
    doubleOuter: 180,
    doubleInner: 170,
  ...
  const sectors = numbers.map((number, i) => {
    const start = ((i * 18 - 98) * Math.PI) / 180; // исправлено направление
  ...
  return (
    <svg width={500} height={500} viewBox="0 0 500 500">
      ...
      {sectors.map(({ number, doublePath, singleOuterPath, triplePath, singleInnerPath }, i) => (
        <g key={i}>
          {/* Двойное кольцо */}
          <path
            d={doublePath}
